...
const missingUserError =
  "Der Nutzer fehlt oder ist nicht fuer diese Aktion freigeschaltet.";
const saveNoteError =
...
export async function createNoteForUser(
  ...
  try {
    const validation = await validateUserRelationships(
      userId,
...
export function createNoteForDefaultDevelopmentUser(input: CreateNoteInput) {
  return createNoteForUser(DEFAULT_DEVELOPMENT_USER_ID, input);
}

export function updateNoteForDefaultDevelopmentUser(input: UpdateNoteInput) {
  return updateNoteForUser(DEFAULT_DEVELOPMENT_USER_ID, input);
}

export function deleteNoteForDefaultDevelopmentUser(noteId: string) {
  return deleteNoteForUser(DEFAULT_DEVELOPMENT_USER_ID, noteId);
}
